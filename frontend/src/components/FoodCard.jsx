...
const FoodCard = ({data}) => {
  ...
  const [quantity,setQuantity]=useState(0)
    const renderStars =(rating) =>{
        ...
        for (let i = 1; i <= 5; i++) {
          stars.push(
           (i<=rating)?(
           <FaStar className="text-yellow-500 text-shadow-lg" />
          ):(
          <FaRegStar className="text-yellow-500 text-shadow-lg" />
        )
          ) 
        }
        return stars;
  ...
  return (
    ...
    <div className=" absolute top-3 right-3 bg-white rounded-full p-1 shadow">
        {data.foodType=="veg"?<FaLeaf  
         className=" text-green-600 text-lg" />:<FaDrumstickBite   className=" text-red-600 text-lg" />}
    </div>
  <img src={data.image} alt="" className='w-full h-full object-cover transition-transform duration-300 hover:scale-105'/>
    ...
    <div className=" flex items-center justify-between mt-auto p-3 ">
      ...
      <div className="flex items-center border rounded-full overflow-hidden shadow-sm ">
         ...
         )):null}} className={`${CardItems.some(i=>i._id == data._id) ?"bg-gray-800":"bg-[#ff4d2d]"} text-white px-3 py-2 transition-colors`}>
